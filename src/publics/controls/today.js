...
(async function startFunction() {
  const koreaSalesData = await util.fetchData(
    `${util.host}/korea/sales?sumType=hour&startDay=${today}&endDay=${nowTime}`,
    "GET"
  );
  console.log(koreaSalesData);
...
async function salesChartKorea(koreaSalesData) {
  ...
  const labelData = koreaSalesData.map((r) => Number(r.payment_hour));
    ...
    <div class="col-3 mb-2">
      <div class="card card-blog card-plain">
        <div class="text-center">
          ...
          <p class="text-sm">${item.quantity}개 | ${util.chunwon(item.sales_price)}천원</p>
        </div>
