...
(async function getLives() {
  ...
  .then((res) => res.json())
  .then((res) => {
    if(res.statusCode === 404) { return console.log(res);}

    document.getElementById("live-title").innerHTML = `${res.live_name}<span class="font-weight-bold text-success text-sm">${res.brand_name}</span>`;
    //Shoplive JS Files
    ...
    let timerId = setTimeout(function tick() {
      ...
      getSales(res.brand_id, start_date, end_date);
...
async function getSales(brand_id, start_date, end_date) {
  ...
  .then((res) => res.json())
  .then((res) => {
    const orderData = res.filter( el => 
      el.order_status_id == 'p1' || el.order_status_id == 'g1'
      || el.order_status_id == 'd1' || el.order_status_id == 'd2'
      || el.order_status_id == 's1' );
...
function liveChart() {
  ...
  if(liveTimeChart) { liveTimeChart.destroy() };
